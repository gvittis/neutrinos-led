...
double chiGabiNorm(int exp, int rule, int np, double *x, double *errors, void* user_data)
 {
  ...
  if(exp==0)
  {
   ...
   for (j=0; j<nexp; j++)
   {
    ...
    chi2 += prior(x[0], 0.0, errors[0]) + prior(x[1], 0.0, errors[1]); 
    }
   }
                          ...
                          double filter_sigma, void *user_data)
{
 ...
 /* see = sin(2*thee);
  ...
  /* Compute P_mm */
  smm = sin(2*thmm);
  P[1][1] = 1 - (pow(smm,2))* pow(sin(Delta41),2);//*exp(-square(2*Delta41)*square(0.1)/(2*pow(E,2)));

  /* Compute P_me */
  sme = sin(2*thme);
  P[1][0] = pow(sme,2) * pow(sin(Delta41),2);//*exp(-square(2*Delta41)*square(0.01)/(2*pow(E,2)));;
...
int main(int argc, char *argv[])
{
  ...
  double true_theta12 = 0.5843;
  double true_theta13 = 0.148;
  double true_theta23 = 0.738;
  double true_deltacp = 0.0;
  double true_sdm = 7.5e-05;
  double true_ldm = 2.4e-03;
...
//  glbInitExperiment("SBN_lar_e.glb",&glb_experiment_list[0],&glb_num_of_exps); 
//  glbInitExperiment("SBN_mboone_e.glb",&glb_experiment_list[0],&glb_num_of_exps);
//  glbInitExperiment("SBN_icarus_e.glb",&glb_experiment_list[0],&glb_num_of_exps); 
  glbInitExperiment("SBN_lar_m.glb",&glb_experiment_list[0],&glb_num_of_exps);
//  glbInitExperiment("SBN_mboone_m.glb",&glb_experiment_list[0],&glb_num_of_exps); 
//  glbInitExperiment("SBN_icarus_m.glb",&glb_experiment_list[0],&glb_num_of_exps);
  ...
  glbSetOscParams(input_errors,0.0, GLB_THETA_EE);   /* Non-standard parameter */
  glbSetOscParams(input_errors,0.0, GLB_THETA_MM);   /* Non-standard parameter */    
  glbSetOscParams(input_errors,0.0, GLB_THETA_ME);   /* Non-standard parameter */
  glbSetOscParams(input_errors,0.0, GLB_DM_41);         /* Non-standard parameter */ 
  ...
  glbSetProjectionFlag(gabi_projection, GLB_FIXED, GLB_THETA_EE); /* Non-standard parameter */
  glbSetProjectionFlag(gabi_projection, GLB_FIXED, GLB_THETA_MM); /* Non-standard parameter */
  glbSetProjectionFlag(gabi_projection, GLB_FIXED, GLB_THETA_ME); /* Non-standard parameter */
  glbSetProjectionFlag(gabi_projection, GLB_FIXED, GLB_DM_41); /* Non-standard parameter */
...
//int rule_rate = glbShowRuleRates(stream, 0, 0, GLB_ALL, GLB_W_EFF, GLB_W_BG, GLB_W_COEFF, GLB_SIG); 
 ...
 /* Compute Chi^2 for all loaded experiments and all rules */
  ...
  char* MYFILE1="./chi-3+1/chi-mm-res.dat"; /* if empty, write to screen, otherwise to file name given here */
  FILE* gabi;
 ...
 double z,y,chi2; 
   ...
   for(z=-4.0;z < 0 - 0.001; z=z+4.0/80)
   for(y=-2.0;y< 2.0+0.01;y=y+4.0/80) 
  {
    glbSetOscParams(test_values, asin(sqrt(pow(10,z)))/2, GLB_THETA_MM);
